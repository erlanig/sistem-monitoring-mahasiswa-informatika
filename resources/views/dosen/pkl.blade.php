...
<div class="p-1 sm:ml-64">
    <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 mt-14">
     <div class="flex items-center justify-center h-20 mb-4 rounded bg-gray-50 dark:bg-gray-800">
        <p class="text-3xl font-bold text-gray-900 dark:text-white">Persetujuan PKL Mahasiswa Perwalian</p>
     </div>
        ...
        <table border="1" class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                <tr class="text-s bg-white border-b dark:bg-gray-800 dark:border-gray-700 text-center">
                    ...
                    <td class="border px-4 py-2">{{ $m->persetujuan_pkl }}</td>
                    <td class="border px-4 py-2">
                        <form method="POST" action="{{ route('dosen.verifikasiPKL', ['nim' => $m->NIM]) }}">
                            @csrf
                            @method('PATCH')
                            <select name="persetujuan_pkl" id="persetujuan_pkl" class="border-2 border-blue-600 rounded-md">
                                <option value="setuju" {{ $m->persetujuan_pkl === 'Disetujui' ? 'selected' : '' }}>Setuju</option>
                                <option value="tidak_setuju" {{ $m->persetujuan_pkl === 'Tidak Disetujui' ? 'selected' : '' }}>Tidak Setuju</option>
                            </select>
