...
<div class="p-4 sm:ml-64">
    <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 mt-14">
       <div class="grid grid-cols-1 gap-4 mb-4">
        ...
        <div>
            <a href="{{ route('departemen.cetakmhstdkaktif', ['tahun' => $tahun, 'status'=> $status]) }}" class="text-white bg-blue-500 hover:bg-blue-600 font-medium text-base text-center py-2 px-4 rounded-full" target="_blank">Cetak File</a>
            ...
            <table class="mt-5 w-full text-sm text-left text-gray-500 dark:text-gray-400">
               ...
               <tbody>
                   @foreach($tdkAktif as $tdkAktif)
                       <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 text-center">
                           <td class="px-6 py-4">{{ $tdkAktif->NIM }}</td>
                           <td class="px-6 py-4">{{ $tdkAktif->nama }}</td>
                           <td class="px-6 py-4">{{ $tdkAktif->angkatan }}</td>
                           <td class="px-6 py-4">{{ $tdkAktif->status }}</td>
                       </tr>
                  @endforeach
               </tbody>
           </table>
